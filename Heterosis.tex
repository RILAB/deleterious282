...
\author[2]{Jeffrey Ross-Ibarra}

\affil[1]{Department of Plant Sciences, University of California, Davis, CA}
\affil[2]{Department of Plant Sciences, Center for Population Biology, and Genome Center, University of California, Davis, CA}
...
% -x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-
Phenotypic data from 247 maize inbred lines from the diversity panel described by \citet{Flint-Garcia2005} were analyzed in the current study (see supplemental data for a list of lines). Each inbred lines was crossed to the stiff-stalk inbred B73 (population A) and both the inbred lines and their B73-hybrids were evaluated in three environments in 2003 \citep{Flint-Garcia2009}. A subset of 102 inbreds were additionally crossed to both B73 (population B1) and Mo17 (population B2) and evaluated in a single environment in 2006 \citep{Flint-Garcia2009}.
...
We made use of genotype data  from \citep{Larsson2013} for the full set of 247 lines %247 correct, not 282?
 lines, available to download from \url{http://www.panzea.org/lit/data_sets.html}. Lines were genotyped using the genotype-by-synthesis approach of \citep[GBS;][]{Elshire2011} approach, resulting in a total of 437,650 SNPs that were partially imputed. Of these SNPs, 127,994 mapped to protein coding sequences representing 123,289 codons in {{\bf \color{red}X}} genes. The median (mean) percentage of missing data per SNP, including triallelic sites, was 1.06\% (2.52\%), while the percentage of heterozygous sites was 1.08\% (2.52\%). Only 4.5\% of SNPs had more than 10\% missing data (Supp Fig~\ref{figureS1}-A), and 0.18\% had more than 10\% heterozygous genotypes (Supp Fig~\ref{figureS1}-B). %were  these SNPs removed?
...
Although most predicted deleterious alleles were rare, 923 were found segregating at high frequency ($\geq 0.80$) across all lines.  To test whether these alleles may have been driven to high frequency by selection during domestication ({\bf \color{red}X}) %cite Lu et al. TIG 2006 here 
we analyzed the pattern of haplotype sharing across the genome \citep{Toomajian2006} within each of the {\bf \color{red} CITE} genetic groups (tropical, stiff-stalk, and non-stiff stalked) as defined by \citet{Flint-Garcia2005}.  Only 87 SNPs ({\bf \color{red} X}\% of all tests) showed signs of positive selection in at least one of the genetic groups, providing little evidence to support linked hitchhiking during domestication as major influence on deleterious alleles in the genome.  %do we see any predicted deleterious that are fixed in all lines? if so we should mention and interpret, as it might contradict this paragraph as written
...
% -x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-
We performed a genome wide association analysis to investigate the role of predicted deleterious alleles in observed levels of heterosis and inbreeding depression.  We looked for associations using all SNPs with a $MAF>0.05$ with {\bf \color{red} X} phenotypes evaluated in two different populations while controlling for population structure (see Methods). %do we need to mention the multiple pops here?
Analyses were carried out using the genetic values of inbred lines and both midparent and best parent heterosis to test the effect of predicted deleterious mutations in inbreeding depression and heterosis.  

Genome wide association results using the genetic values of inbred lines identified between 219 (cob diameter) and 598 (cob length) significant SNPs, but predicted deleterious SNPs showed little evidence for enrichment among significant associations (Tables \ref{popA_sig_enirchment} - \ref{popB_sig_enirchment}). 
%I would combine these two tables. If needed, the #s of SNPs could go elsewhere in supp. but i think we should reduce the number of tables for readers.  And 1 & 2 are saying same thing.
%
% what does the "within genes" part of the table legend mean? the enrichment is done genome-wide or at the gene level?
%
%DELETED: Nonetheless, we observed negative correlations between the number of significantly associated deleterious SNPs and flowering time, plant morphology, seed number and seed weight and the genetic values of inbred lines (Table~\ref{table1} and Table~\ref{table2}). These correlations are relatively high (Table~\ref{table3} and Table~\ref{table4}), but should be interpreted with caution because the total number of significant SNPs is $<40$ in all cases.
%
% I'm tempted to remove the above on correlations, or we need more explanation. we see for example a strong positive correlation b/t number of deleterious and plant yield? that doesn't make sense does it? 
% do we see these same correlations with all signficiant SNPs? 
% worth trying the correlation with all deleterious SNPs (even those w/ MAF<0.05)? 
%
Results of associations between heterozygosity and heterosis showed highly variable numbers of significant loci (Tables \ref{popA_sig_enirchment} - \ref{popB_sig_enirchment}). 
% is it meaningful that cob length had most number of sig. hits for inbreds but one of the lowest for heterosis? does this tell us something about genetic load for cob length? or is it not selected for in hybrids?
Virtually almost all traits exhibited some enrichment ($5-45\%$) of predicted deleterious SNPs among significant heterozygous associations, but only for whole plant yield and days to tasseling was the observed enrichment statistically significant. %I'm confused here, as there are a lot of numbers in the tables that have asterisks by them.  those aren't statistically significant?
%45% enrichment is pretty large, isn't it? do you think we should highlight how big some of those numbers are?

%DELETED: Also, a moderately high correlation was observed, for several traits, between the number of heterozygote complementary alleles (one predicted  deleterious allele and one non deleterious) and hybrid vigor (Table~\ref{table3} and Table~\ref{table4}). However, some of these correlations are not relevant because of the low number of significant loci observed. 
% Here too I think we should delete. I'm not sure how to interpret these correlations, especially since some vary in sign by pop, between inbred vs. hybrid, etc.  I'm happy to keep teh correlations in, but will require some more careful explanation and interpretation.  What should readers take away from these correlations?  Why are they interesting?  

Because many deleterious SNPs are at frequencies too low for inclusion in association analysis, we expanded our test of enrichment to the gene level.  We asked whether genes with predicted deleterious SNPs were more likely than random to have SNPs significantly associated with traits of interest.  At this level we see much stronger evidence of enrichment -- a number of traits show statistically significant enrichment in population A, but all traits in both populations show a positive enrichment for genes with predicted deleterious SNPs (Tables \ref{gene_enrichment_A}-\ref{gene_enrichment_A})), a result that is highly unlikely by chance (sign test p={\color{red} \bf X}). % can do a simpel sign test here. it will be crazy significant.
...
This observation may be due to so-called synthetic associations between rare deleterious loci and a common locus at high enough frequency to be included in the association mapping analyses \citep{Dickson2010,Goldstein2009}. Recent work, however, suggests that this sort of association is only likely to hold for deleterious SNPs with relatively small effect on phenotype ({\color{red} \bf CITE}) %cite Thornton paper. am I interpreting it correctly?  
% I think combine the gene enrichment tables as suggested above.  If we could get down to 2-3 tables and 4 figures that would be good.
...
%I redid the column headers on the tables, as I'm not fond of Enri etc.  I think simple letters are sufficient, but feel free to change if you disagree.
...
\begin{table*}[ht]
  \begin{center}
    \caption[]{Total number of significant SNPs (\emph{S*}) and fold enrichment (\emph{f}) within genes, for loci with deleterious mutations in population A. Numbers marked with * are statistically signficiant.}
{\fontsize{10}{10}\sf
      \begin{tabular}{l|rr|rr|rr} 
\toprule
\multicolumn{1}{c}{}	&	\multicolumn{2}{c}{Inbreds}	&	\multicolumn{2}{c}{BPH}	&	\multicolumn{2}{c}{MPH}	\\	\hline 
Traits	& 	\emph{S*} 	& 	\emph{f} 	& 	\emph{S*} 	& 	\emph{f} 	& 	\emph{S*} 	& 	\emph{f}	\\	\hline \hline 
...
\begin{table*}[ht]
  \begin{center}
    \caption[]{Total number of significant SNPs (\emph{S*}) and fold enrichment (\emph{f}), within genes, for loci with deleterious mutations in population B. Numbers marked with * are statistically signficiant.}
{\fontsize{9}{8}\sf
...
Traits	& 	\emph{S*} 	& 	\emph{f} 	& 	\emph{S*} 	& 	\emph{f} 	& 	\emph{S*} 	& 	\emph{f}	& 	\emph{S*} 	& 	\emph{f} 	& 	\emph{S*} 	& 	\emph{f}	\\	\hline \hline 
...
%\begin{table*}[ht]
%  \begin{center}
%    \caption[]{Pearson correlation coefficient  between the total (\emph{S}) or significantly associated (\emph{S*}) number of predicted deleterious SNPs  with traits in population A} 
%Correct that it should be SNPs not "deleterious sites"?
%{\fontsize{10}{10}\sf
%      \begin{tabular}{l|rr|rr|rr} 
%\toprule
%\multicolumn{1}{c}{}	&	\multicolumn{2}{c}{Inbreds}	&	\multicolumn{2}{c}{BPH}	&	\multicolumn{2}{c}{MPH}	\\	\hline 
%Traits 	& 	\emph{S} 	& 	\emph{S*} 	& 	\emph{S}  	& 	\emph{S*} 	& 	\emph{S}  	& 	\emph{S*}	\\ \hline \hline
...
%\begin{table*}[ht]
%  \begin{center}
%    \caption[]{Pearson correlation coefficient  between the total (\emph{S}) or significantly associated (\emph{S*}) number of predicted deleterious SNPs  with traits in population B}
%{\fontsize{10}{10}\sf
%      \begin{tabular}{l|rr|rr|rr|rr|rr} 
%\toprule
%\multicolumn{1}{c}{}	&	\multicolumn{2}{c}{Inbreds}	&	\multicolumn{2}{c}{BPH\_B73}	&	\multicolumn{2}{c}{MPH\_B73}	&	\multicolumn{2}{c}{BPH\_Mo17}	&	\multicolumn{2}{c}{MPH\_Mo17}	\\	\hline 
%Traits 	& 	\emph{S} 	& 	\emph{S*}  	& 	\emph{S} 	& 	\emph{S*}  	& 	\emph{S} 	& 	\emph{S*} 	& 	\emph{S} 	& 	\emph{S*} & 	\emph{S} 	& 	\emph{S*} 	\\ \hline \hline
