...
%These alleles are in general at very low frequencies and there are few deleterious SNPs differentially fixed among different genetic groups. 
...
%
where $\hat{G}$ is the vector of estimated genetic values for inbred lines, $\mu$ is the mean of $\hat{G}$, $M$ is the tested SNP, $\vartheta$ is the SNP effect, $S$ is the structure covariates estimated by \citet{Flint-Garcia2005}, $\beta$ is the fixed structure effects, $Z$ is an incidence matrix, $u$ is a random effect vector assumed to be $N(0,\sigma_{ {\varepsilon}}^{2}K)$ and $\varepsilon$ are the model residuals assumed to be $N(0,\sigma_{ {\varepsilon}'}^{2}I)$. 
...
\begin{figure*}[!b]
  \begin{center}
   \includegraphics[width=150mm]{SFS.png}
    \caption{Derived allele frequency spectrum of (A) synonymous \emph{vs} non-synonymous SNPs and (b) non-synonymous non-deleterious \emph{vs} non-synonymous deleterious SNPs. \emph{Tripsacum} was used as out-group for identifying the derived allele.} 
\label{sfs_non_syn}
  \end{center}
\end{figure*}
%-------------------------------------------------------------------
...
With allele frequency correction, 287 genes had a predicted deleterious SNP in the top 1\% of Fst among genetic groups, and 30 genes have 2 or more high-Fst predicted deleterious SNPs.  Only eleven genes with high-Fst deleterious SNPs  are, however, found in regions thought to be selected during maize improvement  \citep{Hufford2012}, and only 44 genes show significant signs of positive selection with the PHS statistic, again providing little support for the idea that these SNPs were affected by selection on linked beneficial mutations.
